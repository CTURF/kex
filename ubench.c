...
int main()
{
  ...
  for (long long i = 0;i < TIMINGS;++i) {
    t[i] = cpucycles();
    t2[i] = fp_mulsq_count;
    xISOG_two(&A[i], 1, primes_batchbound[0]);
  }
  printf("xISOG_two cycles");
  for (long long i = 1;i < TIMINGS;++i) printf(" %lld", t[i]-t[i-1]);
  printf("\n");
  printf("xISOG_two mults");
  for (long long i = 1;i < TIMINGS;++i) printf(" %lld", t2[i]-t2[i-1]);
  printf("\n");
  ...
  for (long long i = 0;i < TIMINGS;++i) {
    t[i] = cpucycles();
    t2[i] = fp_mulsq_count;
    to_montgomery_minus_curve(&A[i]);
  }
  printf("to_montgomery_minus_curve cycles");
  for (long long i = 1;i < TIMINGS;++i) printf(" %lld", t[i]-t[i-1]);
  printf("\n");
  printf("to_montgomery_minus_curve mults");
  for (long long i = 1;i < TIMINGS;++i) printf(" %lld", t2[i]-t2[i-1]);
  printf("\n");

  for (long long lpos = 1;lpos < primes_num;++lpos) {
	...
	if (stopearly) break;
	  ...
	  if (stopearly) break;
          ...
          printf("iso_setup %lld", primes[lpos]);
          for (long long i = 1;i < TIMINGS;++i) printf(" %lld",(t[i]-t[i-1]));
