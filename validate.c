...
static int validate_rec(proj *P, proj const *A, long long lower, long long upper, uintbig *order, int *criticaltestdone)
{
    ...
    if (upper - lower == 1)
    {
        ...
        if (*criticaltestdone != 1)
        {
            ...
            xMUL_dac_minus(&Q, A, P, primes_dac[lower], primes_daclen[lower], primes_daclen[lower]);
            if (!fp_iszero(&Q.z))
                return -1;
            *criticaltestdone = 1;
        }
    ...
    for (long long i = lower; i < mid; ++i) {
        xMUL_dac_minus(&Q, A, &Q, primes_dac[i], primes_daclen[i], primes_daclen[i]);
    }
...
int validate_cutofforder_v2(uintbig *order, const fp *P, const fp *A)
{
    ...
    // Extra power of 3 in p + 1
    xDBL_minus(&Pproj0, &Pproj, &Aproj);
    xADD_minus(&Pproj, &Pproj0, &Pproj, &Pproj);

    // Extra powers of 2 in p + 1
    xDBL_minus(&Pproj, &Pproj, &Aproj);
    xDBL_minus(&Pproj, &Pproj, &Aproj);
