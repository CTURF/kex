  ...
  for (long long lpos = 0;lpos < primes_num;++lpos) {
    ...
    if (l == 2)
    {
        for (long long t = 0; t < TIMINGS; ++t)
        {
            ...
            xISOG_two_old(&A[t], 1, primes_batchbound[0]);
            baseline[t] = (cpucycles() - baseline[t]) / primes_batchbound[0];
        }
    }
    else
    {
    ...
    for (long long bs = 0;bs <= 32;bs += 2) {
      for (long long gs = 0;;++gs) {
        ...
        if (l == 2)
        {
            ...
            {
                bench[t] = cpucycles();
                xISOG_two(&A[t], 1, primes_batchbound[0]);
                bench[t] = (cpucycles() - bench[t]) / primes_batchbound[0];
            }
        }
        else
        {
