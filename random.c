...
void random_boundedl1_nonzero(int8_t *e,const long long S)
{
  // There is probably a smarter way to this, but the goal of this
  // implementation is to provide an faster implementation of the group action,
  // not to optimize  every last bit out of secret key generation, which is
  // already quite fast.
  for(;;)
  {
      random_boundedl1(e, 1, S);
      long long reject = e[0] == 0;
      crypto_declassify(&reject, sizeof reject);
      if (reject) {
        continue;
      }
