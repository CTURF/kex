...
void xDBL_minus(proj *Q, proj const *P, const proj *A)
{
  fp xSqr, zSqr, t0, t1;
  ...
  fp_mul3(&t0, &P->x, &P->z); // t0 = XZ
  fp_mul3(&t1, &t0, &A->x); // t1 = AXZ

  fp_add3(&Q->x, &xSqr, &zSqr);
  fp_sq1(&Q->x); // (X^2 + Z^2)^2

  fp_add3(&Q->z, &t1, &xSqr); // Q.Z = AXZ + X^2
  fp_sub2(&Q->z, &zSqr); // Q.Z = AXZ + X^2 - Z^2
  fp_mul2(&Q->z, &t0); // Q.Z = XZ * (AXZ + X^2 - Z^2)
  fp_quadruple1(&Q->z);
}

void xADD_minus(proj *S, proj const *P, proj const *Q, proj const *PQ)
{
    fp a, b, x, z;
...
void xMUL_dac_minus(proj *Q, proj const *A, proj const *P, long long dac, long long daclen, long long maxdaclen)
{
  proj Pinput = *P; // keeping copy since Q can overlap P
  proj P1 = Pinput;
  proj P2; xDBL_minus(&P2,&P1,A);
  proj P3; xADD_minus(&P3,&P2,&P1,&P1);
...
void xISOG_two_old(proj *A, int8_t e, long long batchbound)
{
    long long signflip = -(e >> 7); // 0 for positive, 1 for negative
    fp neg_A;
    fp_neg2(&neg_A, &A->x);

    fp_cmov(&A->x, &neg_A, signflip);
    ...
    int8_t abs_e = e * (1 - 2 * signflip);

    fp t0;
    for (int8_t i = 1; i < batchbound; i++)
    {
        fp_sq2(&eps, &A->x);
        fp_sub3(&eps, &eps, &fp_4);
        fp_sqrt(&eps);
        fp_sub3(&t0, &eps, &A->x);
        fp_mul2(&t0, &A->x);
        fp_add2(&t0, &fp_3);
        fp_add3(&t0, &t0, &t0);
        fp_cmov(&A->x, &t0, i < abs_e);
    }
...
void xISOG_two(proj *A, int8_t e, long long batchbound)
{
    long long signflip = -(e >> 7); // 0 for positive, 1 for negative
    fp neg_X;
    fp_neg2(&neg_X, &A->x);

    fp_cmov(&A->x, &neg_X, signflip);

    fp eps;
    fp_sq2(&eps, &A->x);
    fp_add2(&eps, &fp_4);
    fp_sqrt_no_check(&eps);
    ...
    for (int8_t i = 1; i < batchbound; i++)
    {
        long long do_cmov = i < abs_e;

        fp z_sqr;
        fp_sq2(&z_sqr, &A->z); // z_sqr = Z**2
        fp three_z_sqr;
        fp_double2(&three_z_sqr, &z_sqr); // three_z_sqr = 2Z**2
        fp_add2(&three_z_sqr, &z_sqr); // three_z_sqr = 3Z**2

        fp t0;
        fp_sq2(&t0, &A->x); // t0 = X**2
        fp_sub2(&t0, &three_z_sqr); // t0 = X**2 - 3Z**2
        fp_sub2(&t0, &z_sqr); // t0 = X**2 - 4Z**2
        fp_sqrt_no_check(&t0); // t0 = sqrt(X**2 - 4Z**2)

        fp_sub2(&t0, &A->x); // t0 = sqrt(X**2 - 4Z**2) - X
        fp_mul2(&t0, &A->x); // t0 = X(sqrt(X**2 - 4Z**2) - X)
        fp_add2(&t0, &three_z_sqr); // t0 = X(sqrt(X**2 - 4Z**2) - X) + 3Z**2
        fp_double1(&t0); // t0 = 2(X(sqrt(X**2 - 4Z**2) - X) + 3Z**2)

        fp_cmov(&A->x, &t0, do_cmov);
        fp_cmov(&A->z, &z_sqr, do_cmov);
    }
...
void to_montgomery_minus_curve(proj *A)
{
    fp eps, tmp;
    fp_sub3(&eps, &A->x, &A->z); // eps = X - Z
    fp_add3(&tmp, &A->x, &A->z); // tmp = X + Z
    fp_sub2(&eps, &A->z); // eps = X - 2Z
    fp_add2(&tmp, &A->z); // tmp = X + 2Z
    fp_mul2(&eps, &tmp); // eps = (X - 2Z) * (X + 2Z) = (X**2 - 4 * Z**2)
    fp_sqrt_no_check(&eps); // eps = sqrt(X**2 - 4Z**2)
    fp_sub3(&tmp, &eps, &A->x);

    long long is_square = fp_sqrt(&tmp) == fp_sqrt(&A->z);
    // s = 1 if is_square, else -1

    fp num, den;

    fp_double2(&num, &eps);
    fp_add2(&num, &eps); // num = 3 * eps
    fp_neg2(&tmp, &num); // tmp = -3 * eps
    fp_cmov(&num, &tmp, is_square); // num = -3 * s * eps
    fp_sub2(&num, &A->x); // num = -3 * s * eps - X

    tmp = A->x;
    fp_neg1(&A->x);

    fp_cmov(&A->x, &tmp, is_square);

    fp_add3(&den, &A->x, &eps);
    fp_double1(&eps);
    fp_mul2(&den, &eps);
    fp_sqrt_no_check(&den);
